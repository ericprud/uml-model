...
function UmlModel (modelOptions = {}, $ = null) {
    ...
    constructor (id, name, source, elements, missingElements) {
      Object.assign(this, {
        ...
        getPrimitiveTypes: function () {
          return this.elements.reduce(
            (acc, pkg) => acc.concat(pkg.list('PrimitiveType')), []
          )
        },
        getProperties: function () {
          let ret = {}
          this.getClasses().concat(this.getDatatypes()).forEach(klass => {
  ...
  class PrimitiveType extends Packagable {
    // Parent may be null for automatic primitiveTypes generated by e.g. XSD hrefs.
    constructor (id, references, name, external, parent, comments) {
      super(id, references, name, parent, comments)
      Object.assign(this, {
        get rtti () { return 'PrimitiveType' },
        external
      })
    }
    ...
    diffs (other, render = s => s, seen = []) {
      if (revisiting(seen, this, other)) { return [] }
      let topic = 'PrimitiveType ' + this.id
      if (this.id !== other.id) {
        return [render(topic + ' doesn\'t match id ' + other.id)]
      }
      return super.diffs(other, render, seen)
    ...
    render () {
      return $('<div/>').addClass('uml primitiveType ' + EXPANDED).append(
        renderElement(_ => this.renderTitle(), [], () => null, 'primitiveType')
      )
    }

    summarize () {
      return $('<span/>').addClass('uml primitiveType').append(
        $('<span/>').text('primitiveType').addClass('type primitiveType'),
        $('<span/>').text(this.name).addClass('name')
      )
    }
    ...
    toShExJ (parents = [], options = {}) {
      ...
      // Calling program encouraged to add xmlPrimitiveType attributes.
      if (this.xmlPrimitiveType) {
  ...
  class Classifier extends Packagable {
    ...
    diffs (other, render = s => s, seen = []) {
      if (revisiting(seen, this, other)) { return [] }
      if (this.id !== other.id) {
        return [render(this.rtti() + ' doesn\'t match id ' + other.id)]
      }
      return super.diffs(other, render, seen)
        ...
        .concat(this.aggregation !== other.aggregation ? [
          render(this.rtti() + ' aggregation:' + other.aggregation + ' doesn\'t match ' + this.aggregation)
        ] : [])
    }
  ...
  class Class extends Classifier {
    constructor (id, references, name, generalizations, properties, isAbstract, parent, comments) {
      super(id, references, name, generalizations, properties, isAbstract, parent, comments)
      Object.assign(this, {
  ...
  class Property {
    constructor (id, inClassifier, name, type, lower, upper, association, aggregation, comments) {
      Object.assign(this, {
    ...
    diffs (other, render = s => s, seen = []) {
      ...
      return testRTTI(topic, this, other, render)
        .concat(this.inClassifier.diffs(other.inClassifier, render, seen).map(
          d => render(topic + d)
        ))
        .concat(this.name !== other.name ? [
          render(topic + ' name:' + other.name + ' doesn\'t match ' + this.name)
    ...
    remove (missingElements) {
      ...
      idx = this.inClassifier.properties.indexOf(this)
      if (idx === -1) {
        throw Error('Property ' + this.id + ' does not appear in Class ' + this.inClassifier.id)
      }
      this.inClassifier.properties.splice(idx, 1) // detach prop from references.
    }
    ...
    propToShExJ (options) {
      let valueExpr =
            this.type.rtti === 'PrimitiveType' && this.type.external === true
        ...
        if (!!v && typeof v !== 'string') {
          ...
          if (obj.rtti === 'Property' && k === 'type' && obj.external) {
            ...
            if (v._idref in objs) {
              ...
              objs[v._idref] = new M.PrimitiveType(v._idref, [prop], v._idref, prop, [])
            }
          }
          if (v._idref) {
            if (!(v._idref in objs)) {
    ...
    // get replaced before substitution.
        ...
        let ret = jsonCycles.parse(from, function (key, value) {
          let references = {
            ...
            'Property': ['type', 'inClassifier'],
            'Enumeration': ['references', 'parent'],
            'PrimitiveType': ['references', 'parent'],
  ...
  function fromJSON999 (obj) {
    ...
    function mapElementByIdref (propertyRecord, reference) {
      if (propertyRecord.href) {
        ...
        return primitiveTypes[propertyRecord.href] = new UmlModel.PrimitiveType(propertyRecord.href, [reference], propertyRecord.href, null, propertyRecord.comments)
      }
    ...
    function createPrimitiveType (primitiveTypeId, reference) {
      ...
      return primitiveTypes[primitiveTypeId] = new UmlModel.PrimitiveType(primitiveTypeId, [reference], primitiveTypeRecord.name, reference, primitiveTypeRecord.comments)
    }
